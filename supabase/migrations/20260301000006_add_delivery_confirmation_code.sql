-- ==============================================================================
-- Migration: Add delivery confirmation code & get_order_by_tracking_token RPC
-- 1. Ensure tracking_token exists with secure random UUID default
-- 2. Add delivery_confirmation_code, delivery_confirmed_via, updated_at to orders
-- 3. Create get_order_by_tracking_token RPC with stage-gated code exposure
-- ==============================================================================

-- 1. Ensure tracking_token has secure UUID default
ALTER TABLE orders 
...
-- 2. Add delivery confirmation code, confirmation method, and updated_at columns
ALTER TABLE orders 
ADD COLUMN IF NOT EXISTS delivery_confirmation_code VARCHAR(10),
ADD COLUMN IF NOT EXISTS delivery_confirmed_via VARCHAR(20),
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- 3. Drop existing function to allow updating return columns
DROP FUNCTION IF EXISTS get_order_by_tracking_token(UUID);

-- 4. Recreate get_order_by_tracking_token RPC
