...
function PayrollSummary({ onRecordAttendance, refreshKey }: PayrollSummaryProps) {
  ...
  async function fetchPayrollData() {
    ...
    try {
      ...
      // 2. Fetch attendance records for selected month/year
  ...
  // Combine workers and attendance into unified rows
  const payrollRows: WorkerPayrollRow[] = useMemo(() => {
    ...
    return workers.map((worker) => {
      ...
      const payable = att
        ? calculatePayable(worker.monthly_salary, att.agreed_working_days, att.days_present)
        : null
      return {
        worker,
        attendance: att,
  ...
  // Total payable for the month across all workers with recorded attendance
  const totalMonthlyPayout = useMemo(() => {
    return payrollRows.reduce((sum, row) => sum + (row.payable ?? 0), 0)
  }, [payrollRows])

  const recordedCount = useMemo(() => {
    return payrollRows.filter((r) => r.attendance !== null).length
  }, [payrollRows])
        ...
        >
          ...
          >
            <thead>
              ...
              >
                {['Worker Name', 'Monthly Base Salary', 'Days Present', 'Agreed Working Days', 'Payable Amount', 'Action'].map((h, idx) => (
                  <th
                    key={h}
                    style={{
                      padding: '11px 16px',
            ...
            <tbody>
              {payrollRows.map((row, i) => (
                ...
                >
                  ...
                  {/* Days Present */}
                  <td style={{ padding: '14px 16px', color: row.attendance ? '#2B2420' : '#6B7259' }}>
                    {row.attendance !== null ? (
                      <span>{row.attendance.days_present} days</span>
                    ) : (
                  ...
                  <td style={{ padding: '14px 16px', color: '#6B7259' }}>
                    {row.attendance !== null ? `${row.attendance.agreed_working_days} days` : '—'}
                  </td>
                  ...
                  <td style={{ padding: '14px 16px', textAlign: 'right' }}>
                    <button
                      ...
                      style={{
                        padding: '6px 12px',
                        background: row.attendance ? '#FAF7F2' : '#B8874B',
                        border: row.attendance ? '1px solid #E4DDD1' : 'none',
                        borderRadius: 2,
                        color: row.attendance ? '#4A3728' : '#fff',
                        fontSize: 11,
                        fontWeight: 600,
                      ...
                      onMouseEnter={(e) => {
                        if (row.attendance) {
                          ...
                          e.currentTarget.style.background = '#A3743C'
                        }
                      }}
                      onMouseLeave={(e) => {
                        if (row.attendance) {
                          ...
                          e.currentTarget.style.background = '#B8874B'
                        }
                      }}
                    >
                      {row.attendance ? 'Edit' : 'Record'}
                    </button>
                  </td>
                </tr>
              ))}
            ...
            <tfoot>
              ...
              >
                <td style={{ padding: '14px 16px', color: '#2B2420', textTransform: 'uppercase', fontSize: 11, letterSpacing: '0.08em' }}>
                  Total ({recordedCount}/{payrollRows.length} Recorded)
                </td>
